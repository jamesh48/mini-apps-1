...
module.exports.generateHTML = (req: Request, callback: (error: any, result?: string) => void) => {
  try {
  ...
  if (callback) {
    fs.writeFile("text.txt", stringifiedJSON, function (err: any) {
      if (err) {
        callback(err);
      }
  ...
  responseHTML += stringifiedJSON
    ...
    .map((entry: string, index: number) => {
      entry = entry
        ...
        .join("");
      return index === 0
        ? `<div class='title-entry'> ${entry}</div>`
